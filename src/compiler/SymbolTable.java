    ...
    public void enterPostfixExpression(CParser.PostfixExpressionContext ctx) {
        ...
        if (!ctx.getText().contains("(")&& !ctx.getText().contains("[")){
                if (ctx.primaryExpression().Identifier() != null) {
                    ...
                    current.print_error_type(name,ctx.start.getLine(),ctx.start.getCharPositionInLine());
                }
            }
    ...
    public void enterProgram(CParser.ProgramContext ctx) {
        ...
        for (int i=0 ; i<ctx.functionDefinition().size();i++)
        {
            ...
            if(ctx.functionDefinition(i).declarator().directDeclarator().parameterTypeList()!= null)
            {
                ...
                if(program.print_error_method(name ,ctx.start.getLine() ,ctx.start.getCharPositionInLine())==1){
   ...
   int line = 1;
    ...
    String type_identification(String input) {
        ...
        if (input.matches("[0-9]+")) {
            // Input is a number, so return "int" as the type
            return "int";
        } else if (input.matches("\".*\"")) {
            // Input is a string literal, so return "String" as the type
            return "String";
        } else if (input.matches("'.'")) {
            // Input is a single character enclosed in single quotes, so return "char" as the type
            return "char";
        } else if (input.matches("\\{\\s*'.'\\s*(,\\s*'.'\\s*)*\\}")) {
            // Input is a comma-separated list of characters enclosed in curly braces, so return "char[]" as the type
            return "char array";
        } else if (input.matches("\\{[0-9,\\s]+\\}")) {
            // Input is a comma-separated list of numbers enclosed in curly braces, so return "int[]" as the type
            return "int array";
        } else if (input.matches("\\{\".*\",\\s*\".*\"\\}")) {
            // Input is a comma-separated list of string literals enclosed in curly braces, so return "String[]" as the type
            return "String array";
        }
