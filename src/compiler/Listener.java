...
public class Listener implements CListener {
    int count = 0;
    int tabCounter = 0 ;
    @Override
    ...
    public void enterPostfixExpression(CParser.PostfixExpressionContext ctx) {
        ...
        if(ctx.primaryExpression()!=null && ctx.primaryExpression().Identifier()!=null && ctx.RightParen().size()!=0) {
            tabCounter++;
            for (int i=0 ; i<tabCounter;i++){
                print_tab();
            }
            System.out.print("function call:");
            System.out.print("name: " + ctx.primaryExpression().Identifier().getText());
            tabCounter--;
    ...
    public void enterArgumentExpressionList(CParser.ArgumentExpressionListContext ctx) {
        ...
        for (CParser.AssignmentExpressionContext assignmentExpressionContext : ctx.assignmentExpression()) {
            ...
            System.out.print(" "+assignmentExpressionContext.conditionalExpression().getText()+" (index :"+index+")");
    ...
    public void enterDeclaration(CParser.DeclarationContext ctx) {
        ...
        int size=ctx.declarationSpecifiers().declarationSpecifier().size();
        if(ctx.initDeclaratorList()==null) {
//            for (int i =0 ; i<ctx.declarationSpecifiers().declarationSpecifier().size() ; i++){

            String name = ctx.declarationSpecifiers().declarationSpecifier().get(size-1).getText();
            System.out.print(" name: " + name + "/");
            String type=ctx.declarationSpecifiers().declarationSpecifier(size-2).getText();
            System.out.print(" type: " + type);
//        }

        }
        else{for (CParser.InitDeclaratorContext initDeclaratorContext : ctx.initDeclaratorList().initDeclarator()) {
            String name=initDeclaratorContext.declarator().directDeclarator().Identifier().getText();
            System.out.print("name: " +name+"/");
            String type=ctx.declarationSpecifiers().declarationSpecifier(size-1).getText();
            System.out.print(" type: "+type);
            if(initDeclaratorContext.declarator().directDeclarator().Constant().size()>0){
                System.out.print("/length: ");
                for (TerminalNode terminalNode : initDeclaratorContext.declarator().directDeclarator().Constant()) {
                    System.out.print(terminalNode+" ");
                }
    ...
    public void enterParameterList(CParser.ParameterListContext ctx) {
        ...
        List<CParser.ParameterDeclarationContext> parameters = ctx.parameterDeclaration();
        for (int i = 0; i < parameters.size(); i++) {
            CParser.ParameterDeclarationContext p = parameters.get(i);
            String type = p.declarationSpecifiers().getText();
            String name = p.declarator().directDeclarator().Identifier().getText();
    ...
    public void enterFunctionDefinition(CParser.FunctionDefinitionContext ctx) {
        ...
        if(!name.equals("main")){
            ...
            System.out.print("name: "+name+"/return type= "+type+" {");
            System.out.println(" ");
        }
        else {
